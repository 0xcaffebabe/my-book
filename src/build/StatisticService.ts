...
class StatisticService extends BaseService {
  ...
  public static async generateStatistic(): Promise<StatisticInfo> {
    ...
    const result = await Promise.all([this.getRepositorySize(), this.getCommitStatistic(), this.getWordStatistic()])
  ...
  private static async getCommitStatistic(): Promise<CommitStatistic> {
    ...
    return {
      ...
      linePerDay: Math.ceil(commitList.map(v => v.insertions - v.deletions).reduce((a, b) => a + b, 0))
    }
  }
